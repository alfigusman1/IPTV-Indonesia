...
class AdvancedChannelManager {
  ...
  getCountryName(countryCode) {
    ...
    const map = {};
    Object.entries(IPTV_CONFIG.countries).forEach(([name, code]) => {
      map[code.toUpperCase()] = name;
    });
    return map[countryCode.toUpperCase()] || 'Undefined';
  }
  ...
  parseM3U(content, playlistId) {
    ...
    // CARI PLAYLIST SOURCE BERDASARKAN ID
    const playlist = this.playlistSources.find(p => p.id === playlistId);
    const fallbackCountry = playlist?.country || 'Undefined';
    const fallbackCode = playlist?.countryCode || 'XX';

    const nameRegex = /,\s*(.+?)\s*$/;
    const logoRegex = /tvg-logo=["']([^"']+)["']/i;
    const countryRegex = /tvg-country=["']([^"']+)["']/i;
    const groupRegex = /group-title=["']([^"']+)["']/i;
    const idRegex = /tvg-id=["']([^"']+)["']/i;

    for (let i = 0; i < lines.length; i++) {
      const line = lines[i].trim();

      if (line.startsWith('#EXTINF:')) {
        const name = line.match(nameRegex)?.[1]?.trim() || 'Unknown Channel';
        const logo = line.match(logoRegex)?.[1] || null;
        const rawCountry = line.match(countryRegex)?.[1];
        const group = line.match(groupRegex)?.[1] || 'general';
        const tvgId = line.match(idRegex)?.[1] || '';

        // --- COUNTRY LOGIC (100% FIXED) ---
        let countryName = fallbackCountry;
        let countryCode = fallbackCode;

        if (rawCountry) {
          const codes = rawCountry.split(';').map(c => c.trim().toUpperCase());
          countryCode = codes[0];
        ...
        currentChannel = {
          id: tvgId || `${playlistId}-${channels.length + 1}`,
          name,
          logo,
          country: countryName,
          countryCode: countryCode,
          category: this.mapCategory(group),
          status: 'unknown',
          playlistId,
  ...
  extractQuality(line) {
    const match = line.match(/\b(4K|2160p|1080p|720p|HD|SD|576p|480p|360p)\b/i);
    ...
    if (q.includes('576P')) return '576p';
    if (q.includes('480P')) return '480p';
    if (q.includes('360P')) return '360p';
    if (q === 'HD') return '720p'; // Hanya jika tidak ada resolusi lain
    if (q === 'SD') return 'sd';
  ...
  mapCategory(category) {
    const categoryMap = {
      ...
      // Kategori spesifik dari iptv-org & umum
      adult: 'adult',
      animation: 'kids',
      auto: 'hobby',
      business: 'news',
  ...
  getCountryCode(countryName) {
    const countryCodes = {
      // Dari playlist iptv-org (nama negara sesuai object)
      Global: 'GL',
      International: 'INT',
      Undefined: 'XX',
  ...
  filterChannels(search = '', country = '', category = '') {
    return this.channels.filter(channel => {
      ...
      const matchCountry = !country || channel.country === country; // Sekarang match nama lengkap
  ...
  getCountries() {
    const countryNames = this.channels
      .map(ch => ch.country)
  ...
  exportAsM3U() {
    ...
    this.channels.forEach(channel => {
      const extinf = `#EXTINF:-1 tvg-id="${channel.id}" tvg-logo="${
        channel.logo || ''
      }" group-title="${channel.category}",${channel.name}`;
      m3u += `${extinf}\n${channel.url}\n\n`;
    });
